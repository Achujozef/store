...
//     <div className="flex overflow-x-auto space-x-4 mb-4 scrollbar-hidden"> {/* Increased bottom margin */}
//       {featuredProducts.map(product => (
//         <div key={product.id} className="flex flex-col items-center min-w-max mx-2 border rounded-lg p-4 shadow-sm">
...
//           <p className="text-xl text-green-600">₹{product.actualPrice}</p>
                ...
                <div className="absolute bottom-2 right-2 flex items-center space-x-2">
                  {cart[product.id] > 0 ? (
                    <div className="flex items-center border rounded-lg text-xs">
                      <button onClick={() => handleRemoveClick(product.id)} className="p-1 text-gray-600 hover:bg-gray-200">
                        <FaMinus />
                      </button>
                      <span className="px-2">{cart[product.id]}</span>
                      <button onClick={() => handleAddClick(product.id)} className="p-1 text-gray-600 hover:bg-gray-200">
                        <FaPlus />
                      </button>
                    </div>
                  ) : (
